...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    if TRAIN_NOISE_DETECTION:
        ...
        if RUN_AUTOKERAS_NOISE:
            ...
            if USE_COMPLEX_MODELS:
                ...
                noise_model_new.compile(optimizer=tf.keras.optimizers.Adam.from_config(         {'amsgrad': False,
                        'beta_1': 0.9,
                        'beta_2': 0.999,
                        'epsilon': 1e-07,
    ...
    if RUN_AUTOKERAS_MURMUR:
        ...
        murmur_model_new.fit(murmur_detection_dataset_train, validation_data=murmur_detection_dataset_val, 
                             epochs = MURMUR_EPOCHS, max_queue_size=MAX_QUEUE, class_weight=class_weight, callbacks=[tf.keras.callbacks.EarlyStopping(
            monitor="val_auc",
            min_delta=0,
            patience=20,
            verbose=0,
    ...
    train_decision_dataset = tf.data.Dataset.from_tensor_slices((np.vstack(embs_train), embs_label_train))
    val_decision_dataset = tf.data.Dataset.from_tensor_slices((np.vstack(embs_val), embs_label_val))
    test_decision_dataset = tf.data.Dataset.from_tensor_slices((np.vstack(embs_test), embs_label_test))
    ...
    if RUN_AUTOKERAS_DECISION:
        ...
        murmur_decision_new.fit(train_decision_dataset, max_queue_size=MAX_QUEUE, batch_size=128, validation_data = val_decision_dataset, epochs = MURMUR_DECISION_EPOCHS, class_weight=class_weight, callbacks=[tf.keras.callbacks.EarlyStopping(
                monitor="val_compute_weighted_accuracy",
                min_delta=0,
                patience=30,
                verbose=0,
...
def get_murmur_decision_model():
 ...
 'layers': [{'class_name': 'InputLayer',
  ...
  {'class_name': 'Activation',
   ...
   'name': 'classification_head_2'}],
 'name': 'model',
 'output_layers': [['classification_head_2', 0, 0]]}
    murmur_decision_new = Functional.from_config(config)
...
def get_murmur_model():
    ...
    noise_dense_config = {'activation': 'linear',  'activity_regularizer': None,  'bias_constraint': None, 'bias_initializer': {'class_name': 'Zeros', 'config': {}}, 'bias_regularizer': None,  'dtype': 'float32', 'kernel_constraint': None, 'kernel_initializer': {'class_name': 'GlorotUniform',  'config': {'seed': None}}, 'kernel_regularizer': None, 'name': 'dense', 'trainable': True, 'units': 1, 'use_bias': True}
...
def get_noise_model(): 
    noise_input_config = {'batch_input_shape': (None, NOISE_IMAGE_SIZE[0], NOISE_IMAGE_SIZE[1], 3),  'dtype': 'float32',  'name': 'input_1', 'ragged': False, 'sparse': False }
    noise_cast_to_float_config = {'dtype': 'float32', 'name': 'cast_to_float32', 'trainable': True}
    ...
    noise_model = tf.keras.Sequential([noise_layer_0, noise_layer_1, noise_layer_3,  
...
def get_noise_model_configs(): 
 ...
 'layers': [{'class_name': 'InputLayer',
   'config': {'batch_input_shape': (None, NOISE_IMAGE_SIZE[0], NOISE_IMAGE_SIZE[1], 3),
    'dtype': 'float32',
    'name': 'input_1',
    'ragged': False,
    'sparse': False},
