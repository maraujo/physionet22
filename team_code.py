...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    if TRAIN_NOISE_DETECTION:
        ...
        if RUN_AUTOKERAS_NOISE:
            ...
            noise_model_new.fit(noise_detection_dataset_train, batch_size = batch_size, epochs = NOISE_EPOCHS, callbacks=[early_stopping_noise], validation_data=noise_detection_dataset_val, workers= WORKERS)
    ...
    train_decision_dataset = tf.data.Dataset.from_tensor_slices((np.vstack(embs_train), embs_label_train)).batch(2)
    ...
    if RUN_AUTOKERAS_DECISION:
        ...
        if USE_COMPLEX_MODELS:
            murmur_decision_config = get_murmur_decision_model() 
            murmur_decision_new.compile(optimizer=tf.keras.optimizers.Optimizer.from_config({'name': 'Adam', 'decay':0.0, 'learning_rate': 0.0001,'beta_1': 0.9, 'beta_2': 0.999, 'epsilon': 1e-07, 'amsgrad': False}), loss="binary_crossentropy", metrics=get_all_metrics())
        else:
...
def get_murmur_model():
    ...
    noise_layer_embs = tf.keras.layers.Dense(64, activation="linear")
